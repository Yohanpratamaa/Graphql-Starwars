...
def resolve_delete_character(_, info, id):
    ...
    try:
        ...
        # Hapus relasi dengan kapal terlebih dahulu
...
def resolve_delete_starship(_, info, id):
    ...
    try:
        ...
        # Hapus relasi dengan karakter terlebih dahulu
...
@mutation.field("removeStarshipFromCharacter")
def resolve_remove_starship_from_character(_, info, input):
    conn = get_db_connection()
    try:
        character = conn.execute("SELECT id FROM characters WHERE id = ?", (input["characterId"],)).fetchone()
        starship = conn.execute("SELECT id FROM starships WHERE id = ?", (input["starshipId"],)).fetchone()
        ...
        conn.execute(
            "DELETE FROM character_starships WHERE character_id = ? AND starship_id = ?",
            (input["characterId"], input["starshipId"]),
        )
        conn.commit()
        character = conn.execute(
...
@mutation.field("updateCharacterHomePlanet")
def resolve_update_character_home_planet(_, info, input):
    conn = get_db_connection()
    try:
        character = conn.execute("SELECT id FROM characters WHERE id = ?", (input["characterId"],)).fetchone()
        if not character:
        ...
        if input.get("planetId"):
            planet = conn.execute("SELECT id FROM planets WHERE id = ?", (input["planetId"],)).fetchone()
            if not planet:
                raise Exception(f"Planet dengan ID {input['planetId']} tidak ditemukan.")
        
        conn.execute(
            "UPDATE characters SET home_planet_id = ? WHERE id = ?",
            (input.get("planetId"), input["characterId"]),
        )
        conn.commit()
        updated_character = conn.execute(
            "SELECT id, name, species, home_planet_id FROM characters WHERE id = ?",
